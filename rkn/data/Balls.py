...
class Balls:
    def __init__(self,
                 ...
                 shuffle=False):  # shuffle data for pretraining (needs cache=1)
        print("BALL DATA: LINEAR, nballs", n_balls)
        self.Dx = 4
        self.w = w
        self.h = h
        self.max_vel = 0.1
        ...
        self.transition_matrix = Balls.getTransitionModel(self.b, self.m, self.dt)
    ...
    def init_state(self, steps):
        ...
        self.state = 2 * self.rand.rand(*(self.n_balls, steps, 4)) - 1
        self.state[:, :, 2:] *= self.max_vel
        self.colors = np.zeros((self.n_balls, 3))

        self.r = np.uint8(self.rand.rand(self.n_balls) * 5 + 5)
        self.r[0] = 5  # always set the first ball the same size
    ...
    def simulate(self, steps=100):
        ...
        for t in range(steps - 1):
            self.state[:, t + 1, :] = np.squeeze(np.matmul(np.expand_dims(self.transition_matrix, 0),
                                                           self.state[:, t, :, np.newaxis]))
            self.state[:, t + 1, 2:] += self.dyn_sigma * self.rand.randn(*(self.n_balls, 2,))
